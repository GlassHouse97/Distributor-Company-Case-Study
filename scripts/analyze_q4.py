"""Run Question 4 revenue and margin analysis and create its portfolio SVG."""
...
ANNUAL_PATH = OUTPUTS / "04_annual_revenue_margin.csv"
MONTHLY_PATH = OUTPUTS / "04_monthly_revenue_margin.csv"
SEASONALITY_PATH = OUTPUTS / "04_revenue_seasonality.csv"
VALIDATION_PATH = OUTPUTS / "04_validation_checks.csv"
ANNUAL_SQL_PATH = SQL_DIR / "04_revenue_margin_trends.sql"
MONTHLY_SQL_PATH = SQL_DIR / "04_monthly_revenue_margin.sql"
SEASONALITY_SQL_PATH = SQL_DIR / "04_revenue_seasonality.sql"
VALIDATION_SQL_PATH = SQL_DIR / "04_validation_checks.sql"
DATABASE_PATH = (
    PROJECT_ROOT / "data" / "processed" / "distributor_case_study.sqlite"
)
SUMMARY_PATH = OUTPUTS / "04_analysis_summary.json"
SVG_PATH = (
    PROJECT_ROOT
    / "SOLUTION"
    / "visualizations"
    / "04_revenue_margin_trends.svg"
)
...
def main() -> None:
    ...
    if len(annual) != 7 or annual["reported_months"].ne(12).any():
        raise ValueError("Question 4 requires seven complete annual periods")
    if len(monthly) != 84 or monthly["period"].nunique() != 84:
        raise ValueError("Question 4 requires 84 unique monthly periods")
    ...
    if not validation["status"].eq("PASS").all():
        ...
        raise ValueError(f"Question 4 validation failed: {failed_checks}")
