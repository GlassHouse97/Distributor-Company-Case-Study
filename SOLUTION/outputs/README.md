...
This folder contains small, reproducible result tables generated from the final SQLite database. They are suitable for spreadsheet review and portfolio publication; the 3.7-million-row transaction fact remains in SQLite.
...
`02_04_analysis_summary.json` contains the compact derived metrics used by the portfolio deliverables. Every CSV can be regenerated from the SQL and final database with the analysis scripts in `scripts/`.
